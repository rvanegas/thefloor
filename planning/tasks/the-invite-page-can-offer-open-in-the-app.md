...
1. A paragraph under the call to action —
   `<p class="app"><a href="thefloor://i/${username}/${pin}">Open in the
   app</a> — once you have it.</p>` — with both halves through `escapeHtml`,
   drawn only where `appStoreUrl` is set, since a box with no store link has no
   installs to open.
...
`thefloor://i/${username}/${pin}` on the named page and on no refusal page, and
still no `href=""` in any of the four absence rows.
