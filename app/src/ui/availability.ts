...
/**
 * The qualifier on a channel's idleness: how long since anybody *but you* was
 * in it — "others 2 days ago", "nobody else yet", or nothing.
 *
 * `describeQuiet` above answers for the room, and a room includes you, so a
 * channel you sat in alone this morning reads fresher there than one two other
 * people used yesterday. This is the clause that stops that being the only
 * thing the row says. It is a qualifier and not a replacement: the row still
 * leads with the room's own number, because that is what the sort and the
 * sections are built on, and this narrows it.
 *
 * **Dropped when it would say the same words as the line it sits beside**,
 * which is the common case — a channel other people use is one where the two
 * numbers are the same moment. `beside` is what `describeQuiet` returned, and
 * the comparison is on the words rather than on the numbers deliberately: two
 * stamps four seconds apart are two numbers and one sentence, and it is the
 * sentence the reader is being spared.
 *
 * Null covers three more, none worth a word. A channel nobody has been in at
 * all — the line already says "not used yet", and "nobody else yet" under it
 * would be the same statement twice. An occupied channel, whose caller draws a
 * count instead and never asks. And a server that predates the field, which is
 * not a fact about the channel: the row reverts to the unqualified line it drew
 * before, which is the right line rather than a degraded one.
 *
 * "nobody else yet" rather than "never": the channel is standing and they may
 * yet walk in, which is what the pair-of-contacts channel is for.
 */
export function describeOthers(
  channel: { everUsed?: boolean; lastPresenceByOthers?: number | null },
  now: number,
  beside: string | null
): string | null {
  if (channel.everUsed === false) return null;
  if (channel.lastPresenceByOthers === undefined) return null;
  if (channel.lastPresenceByOthers === null) return 'nobody else yet';
  const gap = ago(now - channel.lastPresenceByOthers);
  return gap === beside ? null : `others ${gap}`;
}

/**
 * The qualifier on a contact's availability: when they were last in one of your
 * channels — "in a channel", "in a channel 2 days ago", or nothing.
 *
 * `describeAvailability` at the top of this file answers where somebody is in
 * the app. `last_seen_at` is stamped by every socket message, so opening the
 * app on a bus and reading nothing is indistinguishable there from an hour of
 * talking. This is the second question, and usually the one the list is being
 * scanned for.
 *
 * The floor is `agoOrNull`'s, and under it the words lose the interval
 * altogether: a stamp inside the last minute, on a map the heartbeat refreshes
 * every few seconds, means they are in the room now, and "in a channel a few
 * seconds ago" would report the age of the snapshot rather than a departure —
 * the same reason `describeAvailability` reads `inApp` first.
 *
 * **Dropped when it would say the same interval as the line beside it**: last
 * seen three hours ago and last in a channel three hours ago means the app-open
 * *was* the channel, and the second clause is the first one again. The
 * comparison rebuilds the sentence `describeAvailability` would have written,
 * which is why the two belong in one file — a copy of that wording anywhere
 * else is one that drifts. Not dropped when the line is "In the app now", which
 * says nothing about channels at all and is precisely the case this exists for.
 *
 * Null is the ways of not knowing, and none gets a word, exactly as in
 * `describeAvailability`: a server that predates the field; an outgoing
 * request — an address rather than a person, whose whereabouts are what must
 * not be revealed; a contact with no channel in common; and a channel you did
 * share that has since ended, which the server holds no longer and so cannot
 * report on. "Never" would be a claim about the last two that the data does not
 * support.
 */
export function describeCompany(
  who: { lastInChannelAt?: number | null },
  now: number,
  beside: string | null
): string | null {
  if (who.lastInChannelAt == null) return null;
  const gap = agoOrNull(now - who.lastInChannelAt);
  if (!gap) return 'in a channel';
  return beside === `Last seen ${gap}` ? null : `in a channel ${gap}`;
}
