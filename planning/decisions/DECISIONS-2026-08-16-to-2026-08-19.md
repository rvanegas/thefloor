# Decisions, 2026-08-16 to 2026-08-19
...
is here to be read and searched, and is otherwise finished: it was split off on
2026-08-20, when the live volume had passed 2,000 lines and a plain read of it
would have stopped there and silently dropped the tail, which in an append-only
file is the newest material.

It runs from the day after the first build went to App Review, where the
previous volume ends, to the day 1.0.0 was approved and build 51 released.
**That is the seam: everything here was written while the only installs were
the author's and a handful of testers'**, with a submitted build to be
compatible with but no strangers holding one. The work reads that way — the
compatibility floor, the update screen, an unnamed channel widening rather than
moving, the meter — all of it landing in a fortnight that master was held back
for, and all of it shipping at once on 2026-08-19 in the first deploy with a
public population on the other end. Everything after that day is in the volume
after this one.
...
What was built, why it was built that way, and what it cost to find out — plus
what was considered and deliberately not built, which is the half most likely to
be mistaken for an oversight.

**On vocabulary**, and on why a channel is never called a room, see the live
volume's header. Both rules were in force throughout this one.
