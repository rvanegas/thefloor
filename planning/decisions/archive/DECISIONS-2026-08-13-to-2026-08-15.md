# Decisions, 2026-08-13 to 2026-08-15
...
2026-08-18, when the live volume had passed 2,000 lines and a plain read of it
would have stopped there and silently dropped the tail, which in an append-only
file is the newest material.

It runs from the day the media server came off LiveKit Cloud, where the first
volume ends, to the day the first build went to App Review. **That is the seam:
everything here is from before this project had a population to be compatible
with.** Nothing in it had to consider what was already on somebody's phone,
because nothing was. Branches and tags were adopted on the last of these days
— "once there was a submitted build to be wrong about" — and everything that
followed from having users out there, the build header on the wire, the
compatibility floor and the update screen that enforces it, is in the volume
after this one.
