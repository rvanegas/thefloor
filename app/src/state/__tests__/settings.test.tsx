...
describe('the settings that follow the account', () => {
  ...
  it('takes what the server says over what this device had cached', async () => {
    ...
    // Left by a build that still had the tap setting. Nothing reads it now;
    // what it is here to prove is that it gets cleaned off the device.
  ...
  it('forgets them at sign-out, and leaves the headset alone', async () => {
    // Left by builds that had the tabs setting and the chime ladder; see the
    // assertions below.
    ...
    // And the chime's, on the same terms — build 211 and earlier cached a
    // chosen peak, and the app has one loudness again. See
    // `DEAD_CHIME_AMPLITUDE_KEY` in AppProvider.tsx.
