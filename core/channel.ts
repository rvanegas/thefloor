 ...
 *
 * **That is settled rather than pending**, decided on 2026-08-26 rather than
 * left as the state the stamps happened to be in. Making a guest count would
 * mean giving a volatile population durable per-identity stamps — a guest id
 * outliving the session it names — to move a number on a list about the people
 * you have accounts with. Home's recency is a claim about members. A guest's
 * visit is visible in the channel, and nowhere on Home, on purpose.
 */
