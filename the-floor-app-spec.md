...
- **Being connected and being in the session are separate things.** A user is in a session until they leave explicitly, the session ends, or they remain disconnected for a **one-minute grace period**. Losing a connection does not remove anyone; it starts that clock, and reconnecting stops it. Sockets may drop and re-establish freely within the minute with no effect on the session at all. A user who is present but unreachable is shown as such to the other party — "Present · reconnecting…" — rather than being made to disappear and reappear.
- **A disconnected user keeps the floor.** It is their claimed time, and it is bounded twice already: by the three-minute expiry, and by the grace period, whose expiry removes them and force-releases the claim like any other leave. Releasing on a dropped connection would punish a bad signal rather than a bad actor.
- **The grace period stacks with the empty-session timer.** If both users disconnect, they have a minute to return before either is removed; only then does the session become empty and the one-minute empty-session timer begin. A session can therefore outlive everyone's connection by up to two minutes, which is deliberate — it is the interval in which a passing train or a dropped call is survivable.
