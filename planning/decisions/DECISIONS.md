...
## A room's recency counts you, and that is the wrong number twice — 2026-08-26

Home ordered and labelled channels by `lastPresenceAt`, which is the maximum
across `lastPresentAt` *and* `lastActiveAt` — so it includes the reader. A
channel you sat in alone this morning read, and sorted, as fresher than one two
other people used yesterday. Contacts had the mirror of it: `lastSeenAt` is
stamped by `markSeen` on every socket message, so somebody who launched the app
on a bus and read nothing was indistinguishable from somebody who had spent an
hour talking. Two numbers were missing — **a channel's activity by others**, and
**a contact's last presence in a channel you share** — and both were computable
from data already held.

**`lastActiveAt` cannot be in the others-fold, and that costs something real.**
It is unattributed: it moves on anybody's entry or exit, including yours, so
admitting it would put the reader's own comings and goings back into the one
number meant to leave them out. `lastPresenceAt` takes the maximum across both
kinds partly *because* the persisted per-person stamps are floored to the minute
by `quantise`, and an exit recorded in `lastActiveAt` can be the fresher
evidence of the very same departure. `lastPresenceByOthers` gives that
correction up, so after a restart it can read up to a minute early. That is
inside the sixty seconds `agoOrNull` already treats as no gap at all, and it
self-corrects at the next heartbeat — but it is a genuine loss and the test
asserts the *bound* rather than a value, so nobody later "fixes" it by folding
`lastActiveAt` back in.

**The contact number is a fold over resident channels, not a column.** Ending a
channel loses nothing — `persistChannel` writes the whole state including
`lastPresentAt`, and the row survives until the sweep. What ending costs is
residency: `restore()` loads `WHERE ended_at IS NULL`, so an ended channel is not
in the map the fold walks, and reaching it would mean a scan of `channels` plus a
`JSON.parse` of each blob on every `GET /home` — a blob the code says is read
whole only at boot. A column buys you out of that read shape and loses on four
counts: it is **global**, so it would report presence in rooms the reader is not
in with nowhere to scope it at read time — the privacy property the fold has by
construction; it needs a migration with no backfill available; its natural stamp
site is the STILL_HERE path, which fires per socket message and would put a row
write every five seconds per present person back into the hot path `quantise`
exists to keep clear; and it is a second source of truth for a fact
`lastPresentAt` already holds. So `lastInChannelAt` means "last in a channel you
*still* share", and the wire comment says so rather than letting a reader widen
it. `lastInChannelFor` walks the channel map once and returns a row per person,
which is what stops the cost being channels × contacts.

**The invitation gets nothing, and the profile gets nothing.** `invitesFor`
skips any channel the viewer is in `everPresent` for, so the viewer's key is by
construction absent from an invited channel's map: the number would equal the
`lastPresenceAt` two lines above it, minus the restart correction. And a profile
already carries `sharedChannels[]` — `present` and `lastPresentAt` per room —
of which this is a maximum the client could take itself; a coarser second
statement that can disagree with the array beside it is worse than none. Both
absences are asserted, so a later session does not add them for symmetry.

**Behind one setting, defaulting off.** Two more intervals on two lists that are
scanned rather than read is a real risk of confusion, and this project's
standing position is that a setting somebody can find is a setting somebody
turns on without knowing what it means. That argues for care about *which*
things get a switch, not against this one: the qualifiers are honest and the
question they answer is the one the lists are usually being scanned for. So the
switch exists and the default is the list people already know.
`thefloor.showOthers` sits in SecureStore beside `tapToStepIn` and the
appearance preference, for the reason they do — how this device's lists read
under a thumb, and two phones signed in as you may reasonably disagree.

**The read is inverted from its neighbour, which is the one subtlety.**
`tapToStepIn` reads as "anything that is not `'false'` is on" because its
default is the behaviour every earlier build had. Here the earlier behaviour is
the qualifier being *absent*, so only the exact string `'true'` turns it on, and
a missing key, a key from a build that never wrote one, and a value nobody
recognises all give the lists as they were.

One key rather than two, though the contact list has a settings screen of its
own: counting other people rather than counting yourself is a single thought,
and splitting it would make somebody find it twice and reconcile the halves.
The card sits in `HomeSettingsView` below the stepping-in one — what a tap does
first, then what a row says — and is two `Button`s, there being no `Toggle` in
`components.tsx` and no reason to introduce one.

**The wire is not gated.** Both fields ship unconditionally and the phone
decides whether to draw them, so toggling is instant with no refetch and the
server is never told what any device has chosen.

**Joined with ` · `, not stacked.** Every card and row on these two screens is a
title plus one `type.muted` line, and every place this codebase puts two facts
on a row it concatenates inside one `<Text>`. A second stacked line is
mechanically free — `rowMain` has `gap: 2` — but `theme.ts` has no tier below
`type.muted` to make it subordinate with, so two muted lines would present the
new number as a *peer* of the old one. It is a qualifier, and the dot says so.
The clause is dropped when it would render the same words as the line it sits
beside, compared as text rather than as numbers: two stamps four seconds apart
are two numbers and one sentence, and it is the sentence the reader is spared.
An occupied channel gets no qualifier at all — its other-stamp heartbeats to
roughly now, so it would read `3 present · others a few seconds ago`. That
leaves one gap, a channel occupied by you alone, which is narrow because the
client draws that one as the live banner rather than as a row; closing it would
need the client to know which present person is itself.

**The sorts are untouched, and that is a decision rather than an omission.**
`byIdleness` and `byAvailability` do not read the preference either, so the
order is the same whichever way it is set — one fewer thing for the setting to
mean. The complaint that opened this was about order as much as about the
label, so the room you were alone in still sits at the top of Home and now says
`5 minutes ago · nobody else yet`, which explains why. Whether it should sort
there is TASKS.md § *Whether Home Sorts On Other People*, and the wording can be
lived with before the order moves under anybody. Two tests pin the current
answer so that changing it is deliberate.

The reassurance worth recording: **every one of the 1,654 tests that predate
this change passes unmodified**, all of them running with the setting off. That
is the check on the default, and it is better than any test that could be
written for it.
